...
const Navbar = () => {
  ...
  return (
    <nav className={`bg-blue-500 shadow-lg sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'shadow-xl' : ''}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo - unique badge + brand */}
          <Link to="/" className="flex items-center space-x-3 group" aria-label="Home">
            <div
              className="flex items-center justify-center gap-2 text-white font-extrabold text-2xl tracking-widetransform transition-transform duration-200 group-hover:scale-105"
              style={{ fontFamily: "Poppins, sans-serif" }}>

              <img src={LogoImage} alt="KitabKart Logo" height="80" width="60" />
              Kitab<span className="text-yellow-400">Kart</span>
            </div>

            <div className="flex flex-col">
              {/* <span className="text-xs text-gray-500 dark:text-gray-300 -mt-1">Organize • Sell • Discover</span> */}
            </div>
          </Link>

          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center space-x-8">
            <Link to="/" className="text-white hover:text-primary-300 transition-colors duration-200 font-medium relative group">
              Home
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-primary-300 group-hover:w-full transition-all duration-200"></span>
            </Link>
            <Link to="/books" className="text-white hover:text-primary-300 transition-colors duration-200 font-medium relative group">
              Books
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-primary-300 group-hover:w-full transition-all duration-200"></span>
            </Link>
            ...
            <form onSubmit={handleSearch} className="flex items-center">
              <div className="relative">
                <input
                  ...
                  className="w-72 pl-12 pr-4 py-2.5 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent shadow-sm hover:shadow-md transition-shadow duration-200"
                />
            ...
            {user && (
              <>
                <Link to="/orders" className="text-white hover:text-primary-300 transition-colors duration-200 font-medium relative group">
                  My Orders
                  <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-primary-300 group-hover:w-full transition-all duration-200"></span>
                </Link>
                {isAdmin && (
                  <Link to="/admin/dashboard" className="text-white hover:text-primary-300 transition-colors duration-200 font-medium relative group">
                    Admin
          ...
          <div className="hidden md:flex items-center space-x-4">
            {user ? (
              <>
                <Link
                  to="/cart"
                  className="relative p-3 text-white hover:text-primary-600 transition-all duration-200 hover:scale-110"
                >
              ...
              <>
                <Link
                  to="/login"
                  className="text-white hover:text-primary-600 transition-colors duration-200 font-medium px-4 py-2 rounded-lg hover:bg-gray-50"
                >
                  Login
                </Link>
                <Link
